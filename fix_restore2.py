...
# Extract raw bytes from git and save as utf-8
output = subprocess.check_output(['git', 'show', 'd09b9fd:app.py'])
...
replacements = {
    ...
    "### ?? Top Khu v?c c ROI L?t sng cao nh?t": "### 💰 Top Khu vực có ROI Lướt sóng cao nhất",
    ...
    "️ **Không có dữ liệu phù hợp.**": "⚠️ **Không có dữ liệu phù hợp.**",
    "Không có khu vực no đạt đủ điều kiện": "Không có khu vực nào đạt đủ điều kiện",
    """except ImportError:
            st.error("Thư viện 'pandasai' chưa được cài đặt. Vui lòng chạy 'pip install pandasai'")""": """except Exception as e:
            import traceback
...
print("Restored and fixed app.py")