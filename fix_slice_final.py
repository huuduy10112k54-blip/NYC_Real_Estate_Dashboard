...
for i, l in enumerate(lines):
    if l.startswith('with tab5:'):
        tab5_index = i
        break

if tab5_index != -1:
    # Everything before tab5
    new_lines = lines[:tab5_index]
...
'''
    ...
    # Append the rest of tab6 from llm = GoogleGemini(...) onwards
    tab6_rest_index = -1
    for i in range(tab5_index, len(lines)):
        if "llm = GoogleGemini(" in lines[i]:
            tab6_rest_index = i
            break
            
    if tab6_rest_index != -1:
        new_lines.extend(lines[tab6_rest_index:])
        
        with io.open('app.py', 'w', encoding='utf-8') as f:
            f.writelines(new_lines)
        print("Replaced chunk successfully!")
    else:
        print("Could not find rest of tab6!")
else:
    print("Could not find start_tab5")