        ...
        try:
            if hasattr(px, 'density_map'):
                fig_map = px.density_map(
                    geo_df, lat='lat', lon='lon', z=target_z, radius=radius_val,
                    center=dict(lat=40.7400, lon=-73.9400), zoom=zoom_val,
                    map_style="open-street-map", color_continuous_scale=color_scale,
                    hover_name="neighborhood",
                    hover_data={"borough_name": True, "med_price": ":$,.0f", "med_ppsf_clean": ":$,.0f", "n_count": ":,", "lat": ":.4f", "lon": ":.4f"},
                    labels={"borough_name": "Quận", "med_price": "Giá trung vị", "med_ppsf_clean": "Giá/sqft", "n_count": "Số GD", "lat": "Latitude", "lon": "Longitude"}
                )
            else:
            ...
            fig_map.update_layout(title_text="", coloraxis_colorbar=dict(title=z_title, len=0.8))
            st.plotly_chart(fig_map, width='stretch')
        except Exception as e:
            st.error(f"Lỗi vẽ Bản đồ Nhiệt: {type(e).__name__} - {str(e)}")
        ...
        for i, row in enumerate(top_3_df.itertuples()):
            with cols[i]:
                ...
                <div style='background-color: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; padding: 15px; text-align: center; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05); border-top: 4px solid #10b981; transition: transform 0.2s;'>
                    ...
                    <div style='font-size: 13px; color: #475569; margin-bottom: 4px;'>Quận: <b>{row[1]}</b></div>
